...
def validate(instance: dict, submission: dict) -> dict:
    ...
    return {'combined_score': float(score), 'candidate_cost': float(total_cost), 'best_known_cost': BEST_KNOWN_COST}
...
def evaluate(candidate: str):
    ...
    with tempfile.TemporaryDirectory(prefix='cvrp_eval_') as tmp:
        ...
        metrics = {
            ...
            'timeout': 0.0,
            'runtime_s': 0.0,
            'candidate_cost': float(scored['candidate_cost']),
            'best_known_cost': float(scored['best_known_cost']),
        }
